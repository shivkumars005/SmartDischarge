$(document).ready(function() {
    ...
    function showNotification(message) {
        const notification = $(`
            <div class="fixed bottom-4 right-4 p-4 rounded-lg shadow-lg animate-fade-in notification-success">
                <span>${message}</span>
    ...
    $('#preview_btn').click(function(e) {
        ...
        if (!patientId) {
            showNotification('Enter a patient ID');
            return;
        }
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.error) {
                    $('#patient_preview').html(`<div class="alert-error p-4 rounded-lg"><strong>⚠️ Error:</strong> ${data.error}</div>`);
                } else {
            ...
            error: function() {
                $('#patient_preview').html(`<div class="alert-error p-4 rounded-lg"><strong>⚠️ Error:</strong> Failed to fetch patient data.</div>`);
            }
        });
    });
                ...
                if (data.error) {
                    ...
                    $('.copy-summary-btn').click(function() {
                        ...
                        navigator.clipboard.writeText(summaryText).then(() => {
                            ...
                            showNotification('Failed to copy summary.');
                        });
                    });
                }
            },
    ...
    $('#summary_form').submit(function(e) {
        e.preventDefault();
        // Trigger generate_btn click to handle submission
        $('#generate_btn').click();
    });
});